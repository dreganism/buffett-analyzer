...
def _as_df(obj) -> pd.DataFrame:
    """Return a DataFrame or an empty DataFrame (avoid 'df or DataFrame()' which is ambiguous)."""
...
def _latest_from_df(df: pd.DataFrame, row_aliases: List[str]) -> Optional[float]:
    """
    Given a Yahoo-style DF (rows = line items, columns = dates), return the most recent value
    ...
    for alias in row_aliases:
        if alias in df.index:
            s = df.loc[alias]
            # Columns are newest -> oldest; use first non-null
            for v in s:
                if pd.notna(v):
                    try:
                        return float(v)
...
def _series_from_df(df: pd.DataFrame, row_aliases: List[str], max_points: int = 8) -> List[float]:
    """
    Return a list (oldest -> newest) of values for the first alias that exists.
    """
    df = _as_df(df)
    if df.empty:
        return []
...
def render_data_quality_flags():
    ...
    try:
        sales = float(s.get("inp_sales") or 0.0)
        ppe = float(s.get("inp_ppe") or 0.0)
        if sales > 1e9 and ppe > 0 and ppe < 1e-5 * sales:
            msgs.append(
                f"Net PP&E {fmt_money_short(ppe)} looks unusually small relative to Sales {fmt_money_short(sales)}. "
                "Verify statement mapping or aliases."
            )
    except Exception:
        pass
    ...
    try:
        re = float(s.get("inp_re")) if s.get("inp_re") is not None else None
        ni = float(s.get("inp_net_income")) if s.get("inp_net_income") is not None else None
        if re is not None and ni is not None and re < -1e9 and ni > 0:
            msgs.append(
...
def fetch_prices_daily(ticker: str, years: int = 10) -> pd.Series:
    ...
    if isinstance(df.columns, pd.MultiIndex):
        # Try selecting the 'Close' panel first
        try:
        ...
        if isinstance(close_panel, pd.DataFrame) and not close_panel.empty:
            # If the ticker-level exists, use it; otherwise first column
            ...
            # Fallback: grab the first numeric column anywhere
...
def fetch_intraday_1m(ticker: str) -> pd.DataFrame:
    # 1 day of 1m bars; Yahoo limits apply by account/IP
    try:
...
def fetch_profile(ticker: str) -> Dict[str, Any]:
    ...
    sector = None
    industry = None
    # yfinance versions differ: prefer .info; .get_info() exists on some versions
    try:
...
def fetch_fundamentals(ticker: str) -> Dict[str, Optional[float]]:
    ...
    total_liabilities = _latest_from_df(bs_a, ["Total Liab", "Total Liabilities", "Total Liabilities Net Minority Interest"])
    ...
    if ppe_net is not None and sales is not None:
        try:
            if abs(ppe_net) > 0 and abs(sales) > 0:
                # If net PPE < 0.00001 * sales and sales > $1B — likely a bad parse
...
def fetch_greenwald_history(ticker: str) -> Dict[str, List[float]]:
    """
    Returns ~5-8 points oldest->newest for:
      sales (revenue), ppe_net, capex
    Uses ANNUAL statements (smoother for Greenwald).
    """
    t = yf.Ticker(ticker)
    is_a = _as_df(getattr(t, "financials", None))
    cf_a = _as_df(getattr(t, "cashflow", None))
...
def fetch_working_capital_quarterly(ticker: str) -> List[float]:
    """
    Returns a list (oldest->newest) of quarterly working capital values.
    """
    t = yf.Ticker(ticker)
    bs_q = _as_df(getattr(t, "quarterly_balance_sheet", None))
