...
# --- Helper: read ENV first, then st.secrets (optional), else default ---
def _cfg(env_key: str, secret_key: Optional[str] = None, default: str = "") -> str:
    """Return configuration value: ENV > st.secrets (optional) > default.
    ...
    if secret_key is None:
        secret_key = env_key
    try:
        v = st.secrets.get(secret_key, default)  # may raise if no secrets file
...
class SubscriptionManager:
    def __init__(self):
        # --- App & DB paths ---
        self.db_path = os.getenv("DATABASE_PATH", "buffett_users.db")
        self.app_url = os.getenv("APP_URL", "http://localhost:8501")

        # --- Stripe config from .env (fallback to secrets via _cfg) ---
        self.stripe_api_key         = _cfg("STRIPE_API_KEY",         "stripe_api_key",         "")
        self.stripe_publishable_key = _cfg("STRIPE_PUBLISHABLE_KEY", "stripe_publishable_key", "")
        self.stripe_webhook_secret  = _cfg("STRIPE_WEBHOOK_SECRET",  "stripe_webhook_secret",  "")
        self.price_ids = {
            "premium":      _cfg("STRIPE_PREMIUM_PRICE_ID",      "stripe_premium_price_id",      "price_premium_default"),
            "professional": _cfg("STRIPE_PROFESSIONAL_PRICE_ID", "stripe_professional_price_id", "price_professional_default"),
        }

        # --- Runtime flags ---
        self.demo_mode  = (_cfg("DEMO_MODE",  "demo_mode",  "true").lower() == "true")
        self.debug_auth = (_cfg("DEBUG_AUTH", "debug_auth", "false").lower() == "true")

        # --- Stripe init & status used elsewhere ---
        self.stripe_configured = bool(self.stripe_api_key)
        if STRIPE_AVAILABLE and self.stripe_api_key:
            stripe.api_key = self.stripe_api_key
        elif STRIPE_AVAILABLE and not self.stripe_api_key and not self.demo_mode:
            st.warning("Stripe installed but STRIPE_API_KEY is not set; running without billing.")
